...
int distance(struct node* ref1, struct node* ref2) {
    ...
    int dist = 0;
    /* Your code here */
	for (struct node* c = ref1; c; c = c->next) {
		if (c == ref2) return dist;
		dist++;
	}

	dist = 0;

	for (struct node* c = ref1; c; c = c->prev) {
		if (c == ref2) return dist;
		dist--;
	}
}
...
struct node* shift_left(struct node* ref, int n) {
    /* Your code here */
	for (int i = 0; i < n; i++, erase(begin(ref)));
...
void minmax(struct node* ref, int* min, int* max) {
    /* Your code here */
	for (struct node* c = ref; c; c = c->next) {
...
bool includes(struct node* ref1, struct node* ref2) {
	...
	for (; ref1; ref1 = ref1->next) {
		if (ref1->data == ref2->data) {
			tmp1 = ref1; tmp2 = ref2;
			for (; ref2; ref1 = ref1->next, ref2 = ref2->next) {
				if (ref1->data != ref2->data) break;
			}
			if (ref2 == NULL) return true;
			ref1 = tmp1; ref2 = tmp2;
		}
	}
    return false;
