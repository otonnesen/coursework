...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    def inbetween(v,end1,end2)
      ...
      (end1-ep <= v and v <= end2 + ep) or (end2-ep <= v and v <= end1+ep)
    end
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    x1start, y1start, x1end, y1end, x2start, y2start, x2end, y2end =
      self.x1, self.y1, self.x2, self.y2, seg.x1, seg.y1, seg.x2, seg.y2
...
class Let < GeometryExpression
  ...
  def eval_prog env
    env2 = []
    (0..env.length-1).each{|i|
      env2[i] = env[i]
    }
    bind = env2.assoc @s
    if bind.nil?
      env2[env.length] = [@s, @e1.eval_prog(env)]
    else
      bind[1] = @e1.eval_prog(env)
    end
    @e2.eval_prog(env2)
  end
end
