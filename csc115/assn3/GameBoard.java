...
public class GameBoard {
	...
	for(int r = 0; r < NUM_ROWS; r++) {
		for(int c = 0; c < NUM_COLS; c++) {
			if(word.charAt(0) == this.board[r][c] && findPath(word, st, r, c)) {
    ...
    private boolean findPath(String wordToFind, Stack<String> pathSoFar, int row, int col) {
	    ...
	    for(int r = 0; r < NUM_ROWS; r++) {
		    for(int c = 0; c < NUM_COLS; c++) {
			    if(debug > 0) {
				    System.out.println("wordToFind: "+wordToFind+", char: "+this.board[r][c]+", row: "+r+", col: "+c);
				    System.out.println(!this.visited[r][c] && wordToFind.charAt(0) == this.board[r][c] && (r-row)*(r-row) <= 1 && (c-col)*(c-col) <= 1);
				    System.out.println(this.board[r][c] == wordToFind.charAt(0));
			    }
			    if(!this.visited[r][c] && wordToFind.charAt(0) == this.board[r][c] && (r-row)*(r-row) <= 1 && (c-col)*(c-col) <= 1) {
				    this.visited[r][c] = true;
				    if(findPath(wordToFind.substring(1), pathSoFar, r, c)) {
				    	    pathSoFar.push("("+r+", "+c+")");
					    return true;
				    } else {
	...
	// System.out.println(test02.isWord("LREKSAJ"));

	Scanner sc = new Scanner(System.in);

	System.out.print("Enter seed: ");
	GameBoard t = new GameBoard(sc.nextInt());
	System.out.print(t+"\nString ('!' to exit): ");
	String x = sc.next();
	while(!x.equals("!")) {
		System.out.println(t.isWord(x));
		System.out.print("String ('!' to exit): ");
		x = sc.next();
	}
	
	// GameBoard g = new GameBoard(214231);
	// Stack<String> st = new Stack<String>();
	// System.out.println(g.findPath("YEYY", st, 0, 0));
    }
}
