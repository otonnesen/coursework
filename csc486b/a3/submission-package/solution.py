...
def train(x_tr, y_tr, x_va, y_va, config):
    ...
    # between this range.
    W = np.random.rand(x_tr_n.shape[1], num_class)
    W = (W-0.5)*0.002
    ...
    for idx_epoch in range(num_epoch):
        ...
        for idx_batch in range(num_batch):
            ...
            x_b = x_tr_n[idx_batch*batch_size:(idx_batch+1)*batch_size]
            y_b = y_tr[idx_batch*batch_size:(idx_batch+1)*batch_size]
...
def main(config):
    ...
    # don't get mixed up!
    perm = np.random.permutation(x_trva.shape[0])
    x_trva = x_trva[perm]
    y_trva = y_trva[perm]
    ...
    for idx_va_fold in va_fold_to_test:
        ...
        if config.cross_validate:
            x_folds = np.split(x_trva, num_fold)
            y_folds = np.split(y_trva, num_fold)
            x_tr = np.concatenate(x_folds[:idx_va_fold]+x_folds[idx_va_fold+1:])
            x_va = x_folds[idx_va_fold]
            y_tr = np.concatenate(y_folds[:idx_va_fold]+y_folds[idx_va_fold+1:])
            y_va = y_folds[idx_va_fold]
        else:
            x_tr = x_trva
            x_va = x_te
            y_tr = y_trva
    ...
    if config.cross_validate:
        ...
        val_acc = np.mean([_res["best_acc"] for _res in train_res], axis=0)
